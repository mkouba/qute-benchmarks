...
public class JsonEscaping {
    ...
    public void prepareDataSet() {
        ...
        this.escaper = Engine.builder()
                .addDefaults()
                .addResultMapper(instantiateJsonEscaper())
                .build()
                .parse("{this}", Variant.forContentType(Variant.APPLICATION_JSON));
        nextInput = 0;
    }
    ...
    private String generateString(Random rnd, byte[] specialChars, byte[] specialCtrlChars, int[] nonSpecialChars,
    ...
    private ResultMapper instantiateJsonEscaper() {
        Class<?> jsonEscaperClass;
        try {
            jsonEscaperClass = JsonEscaping.class.getClassLoader().loadClass("io.quarkus.qute.JsonEscaper");
            return (ResultMapper) jsonEscaperClass.getDeclaredConstructor().newInstance();
        } catch (Exception e) {
            throw new IllegalStateException("Unable to instantiate JsonEscaper", e);
        }
